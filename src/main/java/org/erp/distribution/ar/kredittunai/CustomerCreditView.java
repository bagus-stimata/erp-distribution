...
public class CustomerCreditView extends CustomComponent {
	...
	private TextField fieldSearchBySJPenagihan = new TextField();
...
//		table.addActionHandler(this);		
		...
		searchComboArea.setContainerDataSource(model.getBeanItemContainerArea());
		searchComboArea.setItemCaptionMode(ItemCaptionMode.EXPLICIT_DEFAULTS_ID);
		searchComboArea.setNullSelectionAllowed(true);
		searchComboArea.setFilteringMode(FilteringMode.CONTAINS);

		searchComboSubArea.setContainerDataSource(model.getBeanItemContainerSubArea());
		searchComboSubArea.setItemCaptionMode(ItemCaptionMode.EXPLICIT_DEFAULTS_ID);
		searchComboSubArea.setNullSelectionAllowed(true);
		searchComboSubArea.setFilteringMode(FilteringMode.CONTAINS);

		searchComboSalesman.setContainerDataSource(model.getBeanItemContainerSalesman());
		searchComboSalesman.setItemCaptionMode(ItemCaptionMode.EXPLICIT_DEFAULTS_ID);
...
//		fieldSearchByNamaSalesman.setValue("");
	...
	public void buildWindowRevisiPlusMinus(FtSalesh itemInvoice){
		VerticalLayout layout = new VerticalLayout();
		
		revisiNotaModel = new RevisiNotaModel(itemInvoice);
		revisiNotaView = new RevisiNotaView(revisiNotaModel);
		revisiNotaPresenter = new RevisiNotaPresenter(revisiNotaModel, revisiNotaView);
		revisiNotaView.setSizeFull();
		layout.addComponent(revisiNotaView);
		
		windowRevisiPlusMinus.setModal(true);
		windowRevisiPlusMinus.center();
		windowRevisiPlusMinus.setWidth("750px");
		windowRevisiPlusMinus.setHeight("270px");
        windowRevisiPlusMinus.setResizable(false);
        windowRevisiPlusMinus.setClosable(false);

		windowRevisiPlusMinus.setContent(layout);
		
		System.out.println("Windows plus minus");
		try{
			getUI().addWindow(windowRevisiPlusMinus);
		} catch(Exception ex){}
	}
	public void destroyWindowRevisiPlusMinus(){
		windowRevisiPlusMinus.close();
	}
