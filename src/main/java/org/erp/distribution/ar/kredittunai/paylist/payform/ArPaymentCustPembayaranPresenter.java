...
//				newNomorUrut = model.getTransaksiHelper().getNewNomorUrutArPayment(divisionBean);
				...
				//1. LENGKAPI HEADER
				model.getArPaymentHeader().setNorek(newNomorUrut);
				
				model.getArPaymentHeader().setClosing(false);
				model.getArPaymentHeader().setEndofday(false);
				model.getArPaymentHeader().setPrintcounter(0);
				model.getArPaymentHeader().setUserid("admin");
				
				model.getArPaymentHeader().setEntrydate(new Date());
				if (model.getTanggalPembayaranManual() !=null){
					model.getArPaymentHeader().setTrdate(model.getTanggalPembayaranManual());
				} else {
					model.getArPaymentHeader().setTrdate(model.getTransaksiHelper().getCurrentTransDate());
				}
...
//				newTransfer = model.getBukuTransferService().findByIdPk(model.getArPaymentDetail().getBukutransferBean().getId());	
			...
			try{
				FtSalesh newRetur = new FtSalesh();
				newRetur = model.getArInvoiceService().findById(model.getArPaymentDetail().getReturBean().getRefno());	
			...
			if (model.getFormOperationStatus().equals(EnumOperationStatus.ADDING.getStrCode())) {
				...
				arpaymentdetailPK.setRefnopayment(model.getArPaymentHeader().getRefno());
				arpaymentdetailPK.setRefnosales(model.getArInvoice().getRefno());
