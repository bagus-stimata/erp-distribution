...
//						view.getTable().removeItem(model.item);
	...
	public void initListenerSubWindowRevisiNota(){
		ClickListener listenerSave=new ClickListener() {				
			public void buttonClick(ClickEvent event) {
				view.destroyWindowRevisiPlusMinus();
				model.getTableBeanItemContainer().addItem(view.getRevisiNotaModel().getFtSalesh());
				view.getTable().refreshRowCache();
				//SUPAYA PENGURUTAN TIDAK KACAU
//				view.getBtnSearch().click();
				
			}
		};
		view.getRevisiNotaView().getBtnSaveForm().addClickListener(listenerSave);
		...
		view.getRevisiNotaView().getBtnCancelForm().addClickListener(listenerCancel);
...
//			searchForm();
			...
			initListenerSubWindowRevisiNota();			
