...
public class PrincipalAreaPresenter implements ClickListener, ValueChangeListener, ItemClickListener{
	private PrincipalAreaModel model;
	private PrincipalAreaView view;
	
	public PrincipalAreaPresenter(PrincipalAreaModel model, PrincipalAreaView view){
		this.model = model;
		this.view = view;
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getpAreaJpaService().removeObject(model.getItemHeader());
...
//		}
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getpAreaJpaService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getpAreaJpaService().updateObject(model.getItemHeader());
...
//		model.getBeanItemContainerBukugiro().addContainerFilter(filter1);
	...
	public PrincipalAreaView getView() {
		return view;
	}
	public void setView(PrincipalAreaView view) {
		this.view = view;
	}
