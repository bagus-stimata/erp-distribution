...
public class PrincipalRegionPresenter implements ClickListener, ValueChangeListener, ItemClickListener{
	private PrincipalRegionModel model;
	private PrincipalRegionView view;
	
	public PrincipalRegionPresenter(PrincipalRegionModel model, PrincipalRegionView view){
		this.model = model;
		this.view = view;
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new PRegion();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getpRegionJpaService().removeObject(model.getItemHeader());
...
//		}
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getpRegionJpaService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getpRegionJpaService().updateObject(model.getItemHeader());
...
//		model.getBeanItemContainerBukugiro().addContainerFilter(filter1);
	...
	public PrincipalRegionView getView() {
		return view;
	}
	public void setView(PrincipalRegionView view) {
		this.view = view;
	}
