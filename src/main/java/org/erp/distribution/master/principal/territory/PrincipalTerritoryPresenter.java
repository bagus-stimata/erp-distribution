...
public class PrincipalTerritoryPresenter implements ClickListener, ValueChangeListener, ItemClickListener{
	private PrincipalTerritoryModel model;
	private PrincipalTerritoryView view;
	
	public PrincipalTerritoryPresenter(PrincipalTerritoryModel model, PrincipalTerritoryView view){
		this.model = model;
		this.view = view;
	...
	public void itemClick(ItemClickEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new PDistributor();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new PDistributor();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getpDistributorJpaService().removeObject(model.getItemHeader());
...
//		}
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getpDistributorJpaService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getpDistributorJpaService().updateObject(model.getItemHeader());
...
//		model.getBeanItemContainerBukugiro().addContainerFilter(filter1);
	...
	public PrincipalTerritoryView getView() {
		return view;
	}
	public void setView(PrincipalTerritoryView view) {
		this.view = view;
	}
