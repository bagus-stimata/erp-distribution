...
public class WarehousePresenter implements ClickListener, ValueChangeListener, ItemClickListener{
	private WarehouseModel model;
	private WarehouseView view;
	
	public WarehousePresenter(WarehouseModel model, WarehouseView view){
		this.model = model;
		this.view = view;
	...
	public void itemClick(ItemClickEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new FWarehouse();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new FWarehouse();
			...
			model.newItemHeader.setGudangutama(true);
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getfWarehouseJpaService().removeObject(model.getItemHeader());
...
//		}
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getfWarehouseJpaService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getfWarehouseJpaService().updateObject(model.getItemHeader());
