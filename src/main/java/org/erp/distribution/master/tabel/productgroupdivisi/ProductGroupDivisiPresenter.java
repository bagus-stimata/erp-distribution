...
public class ProductGroupDivisiPresenter implements ClickListener, ValueChangeListener, ItemClickListener{
	private ProductGroupDivisiModel model;
	private ProductGroupDivisiView view;
	
	public ProductGroupDivisiPresenter(ProductGroupDivisiModel model, ProductGroupDivisiView view){
		this.model = model;
		this.view = view;
	...
	public void itemClick(ItemClickEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new FProductgroupdivisi();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new FProductgroupdivisi();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getfProductgroupdivisiJpaService().removeObject(model.getItemHeader());
...
//		}
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getfProductgroupdivisiJpaService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getfProductgroupdivisiJpaService().updateObject(model.getItemHeader());
...
//		model.getBeanItemContainerBukugiro().addContainerFilter(filter1);
	...
	public void setModel(ProductGroupDivisiModel model) {
		this.model = model;
	}
	public ProductGroupDivisiView getView() {
		return view;
	}
	public void setView(ProductGroupDivisiView view) {
		this.view = view;
	}
