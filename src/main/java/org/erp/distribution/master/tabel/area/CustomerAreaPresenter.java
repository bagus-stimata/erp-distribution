...
public class CustomerAreaPresenter implements ClickListener, ValueChangeListener, ItemClickListener{
	private CustomerAreaModel model;
	private CustomerAreaView view;
	
	public CustomerAreaPresenter(CustomerAreaModel model, CustomerAreaView view){
		this.model = model;
		this.view = view;
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getfAreaJpaService().removeObject(model.getItemHeader());
...
//		}
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getfAreaJpaService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getfAreaJpaService().updateObject(model.getItemHeader());
...
//		model.getBeanItemContainerBukugiro().addContainerFilter(filter1);
	...
	public CustomerAreaModel getModel() {
		return model;
	}
	public void setModel(CustomerAreaModel model) {
		this.model = model;
	}
	public CustomerAreaView getView() {
		return view;
	}
	public void setView(CustomerAreaView view) {
		this.view = view;
	}
