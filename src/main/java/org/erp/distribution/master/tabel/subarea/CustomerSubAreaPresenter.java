...
public class CustomerSubAreaPresenter implements ClickListener, ValueChangeListener, ItemClickListener{
	private CustomerSubAreaModel model;
	private CustomerSubAreaView view;
	
	public CustomerSubAreaPresenter(CustomerSubAreaModel model, CustomerSubAreaView view){
		this.model = model;
		this.view = view;
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new FSubarea();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getfSubareaJpaService().removeObject(model.getItemHeader());
...
//		}
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getfSubareaJpaService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getfSubareaJpaService().updateObject(model.getItemHeader());
...
//		model.getBeanItemContainerBukugiro().addContainerFilter(filter1);
	...
	public CustomerSubAreaView getView() {
		return view;
	}
	public void setView(CustomerSubAreaView view) {
		this.view = view;
	}
