...
public class CustomerGroupPresenter implements ClickListener, ValueChangeListener, ItemClickListener{
	private CustomerGroupModel model;
	private CustomerGroupView view;
	
	public CustomerGroupPresenter(CustomerGroupModel model, CustomerGroupView view){
		this.model = model;
		this.view = view;
	...
	public void itemClick(ItemClickEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new FCustomergroup();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new FCustomergroup();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getfCustomergroupJpaService().removeObject(model.getItemHeader());
...
//		}
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getfCustomergroupJpaService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getfCustomergroupJpaService().updateObject(model.getItemHeader());
...
//		model.getBeanItemContainerBukugiro().addContainerFilter(filter1);
	...
	public CustomerGroupModel getModel() {
		return model;
	}
	public void setModel(CustomerGroupModel model) {
		this.model = model;
	}
	public CustomerGroupView getView() {
		return view;
	}
	public void setView(CustomerGroupView view) {
		this.view = view;
	}
