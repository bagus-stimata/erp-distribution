...
public class CustomerSubGroupPresenter implements ClickListener, ValueChangeListener, ItemClickListener{
	private CustomerSubGroupModel model;
	private CustomerSubGroupView view;
	
	public CustomerSubGroupPresenter(CustomerSubGroupModel model, CustomerSubGroupView view){
		this.model = model;
		this.view = view;
	...
	public void itemClick(ItemClickEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new FCustomersubgroup();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new FCustomersubgroup();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getfCustomersubgroupJpaService().removeObject(model.getItemHeader());
...
//		}
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getfCustomersubgroupJpaService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getfCustomersubgroupJpaService().updateObject(model.getItemHeader());
...
//		model.getBeanItemContainerBukugiro().addContainerFilter(filter1);
	...
	public CustomerSubGroupModel getModel() {
		return model;
	}
	public void setModel(CustomerSubGroupModel model) {
		this.model = model;
	}
	public CustomerSubGroupView getView() {
		return view;
	}
	public void setView(CustomerSubGroupView view) {
		this.view = view;
	}
