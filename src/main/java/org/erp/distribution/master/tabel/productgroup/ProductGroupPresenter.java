...
public class ProductGroupPresenter implements ClickListener, ValueChangeListener, ItemClickListener{
	private ProductGroupModel model;
	private ProductGroupView view;
	
	public ProductGroupPresenter(ProductGroupModel model, ProductGroupView view){
		this.model = model;
		this.view = view;
	...
	public void itemClick(ItemClickEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new FProductgroup();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new FProductgroup();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getfProductgroupJpaService().removeObject(model.getItemHeader());
...
//		}
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getfProductgroupJpaService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getfProductgroupJpaService().updateObject(model.getItemHeader());
...
//		model.getBeanItemContainerBukugiro().addContainerFilter(filter1);
	...
	public void setView(ProductGroupView view) {
		this.view = view;
	}
