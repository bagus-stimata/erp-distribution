...
public class ProductGroupDeptPresenter implements ClickListener, ValueChangeListener, ItemClickListener{
	private ProductGroupDeptModel model;
	private ProductGroupDeptView view;
	
	public ProductGroupDeptPresenter(ProductGroupDeptModel model, ProductGroupDeptView view){
		this.model = model;
		this.view = view;
	...
	public void itemClick(ItemClickEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new FProductgroupdept();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new FProductgroupdept();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getfProductgroupdeptJpaService().removeObject(model.getItemHeader());
...
//		}
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getfProductgroupdeptJpaService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getfProductgroupdeptJpaService().updateObject(model.getItemHeader());
...
//		model.getBeanItemContainerBukugiro().addContainerFilter(filter1);
	...
	public ProductGroupDeptModel getModel() {
		return model;
	}
	public void setModel(ProductGroupDeptModel model) {
		this.model = model;
	}
	public ProductGroupDeptView getView() {
		return view;
	}
	public void setView(ProductGroupDeptView view) {
		this.view = view;
	}
