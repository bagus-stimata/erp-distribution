...
public class STeknisiView extends CustomComponent{
	...
	public STeknisiView(STeknisiModel model){
		this.model = model;
		initComponent();
		buildView();
		initComponentState();
...
//		layoutTable.setMargin(true);
		...
		formLayout.addComponent(fieldFeePerService);
		formLayout.addComponent(fieldPercentFeePerService);
...
//		comboGrup.setNullSelectionAllowed(false);
		...
		model.getBinderHeader().bind(fieldId, "nip");
		...
		model.getBinderHeader().bind(fieldFeePerService, "feePerService");
		model.getBinderHeader().bind(fieldPercentFeePerService, "percentFeePerService");
	...
	public void setTableProperties(){
		...
		setVisibleTableProperties("id", "nip","name" , "address1", "city1",
				"state1", "phone1", "email", "joindate", "lasttrans", "statusactive");
...
//		table.setColumnAlignment("amountused", Align.RIGHT);
		...
		//set header
		table.setColumnHeader("nip", "CUSTNO");
		table.setColumnHeader("name", "NAMA CUSTOMER");
