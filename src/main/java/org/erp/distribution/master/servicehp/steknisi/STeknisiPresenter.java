...
public class STeknisiPresenter implements ClickListener, ValueChangeListener, ItemClickListener{
	private STeknisiModel model;
	private STeknisiView view;
	
	public STeknisiPresenter(STeknisiModel model, STeknisiView view){
		this.model = model;
		this.view = view;
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new STeknisi();
			//Karena tidak pisa pake null representation
			
			model.newItemHeader.setNip("New");
			model.newItemHeader.setName("");
			...
			model.newItemHeader.setFeePerService(0.0);
			model.newItemHeader.setPercentFeePerService(0.0);
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getsTeknisiJpaService().removeObject(model.getItemHeader());
...
//		}
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getsTeknisiJpaService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getsTeknisiJpaService().updateObject(model.getItemHeader());
...
//		fd.extend(view.getBtnPrint());
	...
	public STeknisiModel getModel() {
		return model;
	}
	public void setModel(STeknisiModel model) {
		this.model = model;
	}
	public STeknisiView getView() {
		return view;
	}
	public void setView(STeknisiView view) {
		this.view = view;
	}
