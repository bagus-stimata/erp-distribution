...
public class WarehouseCoveredItemView extends CustomComponent {
	/**
	 * 
	 */
	...
	private ComboBox comboWarehouse= new ComboBox("WAREHOUSE");//		model.getBinderItemDetail().bind(fieldNomorUrut, "nourut");
	...
	private Panel panelComboWarehouse = new Panel("WAREHOUSE");
	...
	public WarehouseCoveredItemView(WarehouseCoveredItemModel model){
		this.model = model;
		initComponent();
		buildView();
...
//		layoutTop.addComponent(fieldNomorUrut);
		panelComboWarehouse.setContent(comboWarehouse);
		layoutTop.addComponent(panelComboWarehouse);
...
//		layoutTop.setComponentAlignment(btnClose, Alignment.BOTTOM_CENTER);
	...
	public void bindAndBuildFieldGroupComponent(){
		...
		comboWarehouse.setNewItemsAllowed(false);
...
//		model.getBinderItemDetail().bind(comboProduct, "fproductBean");
		...
		model.getBinderItemDetail().bind(fieldAlamat, "address1");
		model.getBinderItemDetail().bind(fieldKota, "city1");
	...
	public WarehouseCoveredItemModel getModel() {
		return model;
	}
	...
	public void setModel(WarehouseCoveredItemModel model) {
		this.model = model;
	}
	...
	public void setPanelComboVendor(Panel panelComboVendor) {
		this.panelComboWarehouse = panelComboVendor;
	}
