...
public class DivisionPresenter implements ClickListener, ValueChangeListener, ItemClickListener{
	...
	public void initListener(){
		...
		//##VENDOR COVERED
		view.getBtnAddWarehouse().addClickListener(this);
		view.getBtnRemoveWarehouse().addClickListener(this);
		...
		ItemClickListener tabelVendorCoveredDetail = new ItemClickListener() {
			...
			public void itemClick(ItemClickEvent event) {
				...
				Item item = view.getTableWarehouseCoverDetail().getItem(itemId);
				boolean entitySelected = item != null;
				
				if (entitySelected) {
					model.fWarehouseSelected = model.getBeanItemContainerWarehouseCovered().getItem(itemId).getBean();
				}	
			}
		};
		view.getTableWarehouseCoverDetail().addItemClickListener(tabelVendorCoveredDetail);

	}
	public void initListenerWindowWarehouseCovered(){
		view.getItemWarehouseCoveredView().getBtnAddAndSave().addClickListener(this);
		view.getItemWarehouseCoveredView().getBtnReset().addClickListener(this);
		view.getItemWarehouseCoveredView().getBtnClose().addClickListener(this);
	...
	public void itemClick(ItemClickEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new FDivision();
			...
			model.getBeanItemContainerWarehouseCovered().removeAllItems();
			model.getBeanItemContainerWarehouseCovered().addAll(model.itemHeader.getFwarehouseSet());
...
//			discardForm(operationStatus);CECEP
			...
			helpForm();
		}else if (event.getButton() == view.getBtnAddWarehouse()){
			view.showWindowVendorCovered();			
			initListenerWindowWarehouseCovered();
			
		}else if (event.getButton() == view.getBtnRemoveWarehouse()){
			model.getBeanItemContainerWarehouseCovered().removeItem(model.fWarehouseSelected);
			view.setDisplayVendorCovered();
			
			
		}else if (event.getButton() == view.getItemWarehouseCoveredView().getBtnAddAndSave()){
			model.getBeanItemContainerWarehouseCovered().addItem(view.getItemWarehouseCoveredModel().getItemDetil());
		}else if (event.getButton() == view.getItemWarehouseCoveredView().getBtnReset()){
		}else if (event.getButton() == view.getItemWarehouseCoveredView().getBtnClose()){
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new FDivision();
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getfDivisionJpaService().removeObject(model.getItemHeader());
	...
	public int insertAndValidateForm(){
		...
		if (kodeValid && inputValid) {
			try {				
				Collection itemIdsVendor = model.getBeanItemContainerWarehouseCovered().getItemIds();
				Set<FWarehouse> itemIdsWarehouse = new HashSet<FWarehouse>();
				for (Object itemId : itemIdsVendor) {
					FWarehouse domain = new FWarehouse();
					domain = model.getBeanItemContainerWarehouseCovered().getItem(itemId).getBean();
					domain.setFdivisionBean(model.newItemHeader);				
					itemIdsWarehouse.add(domain);
				}
				model.newItemHeader.setFwarehouseSet(itemIdsWarehouse);
				
				//3.1 commit form >> MASUKKAN KE DATABASE
				model.getBinderHeader().commit(); //COMMIT TETAP PAKE NEW TAPI CLIENT SIDE
				
				model.getfDivisionJpaService().createObject(model.getNewItemHeader());
				
				//UPDATE MANUAL WAJIB HUKUMNYA
				for (FWarehouse fWarehouse: itemIdsWarehouse) {
					FWarehouse domain = new FWarehouse();
					domain = fWarehouse;
					domain.setFdivisionBean(model.newItemHeader);
					model.getfWarehouseJpaService().updateObject(domain);
				}
	...
	public int updateAndValidateForm(){
		try{
			Collection itemIdsWarehouse = model.getBeanItemContainerWarehouseCovered().getItemIds();
			Set<FWarehouse> setFWarehouse = new HashSet<FWarehouse>();
			for (Object itemId : itemIdsWarehouse) {
				FWarehouse domain = new FWarehouse();
				domain = model.getBeanItemContainerWarehouseCovered().getItem(itemId).getBean();
				domain.setFdivisionBean(model.itemHeader);				
				setFWarehouse.add(domain);
			}
			model.itemHeader.setFwarehouseSet(setFWarehouse);
			

			//3.2 Masukkan nilai form yang valid(commit) ke dalam tabel >> dan database
			model.getBinderHeader().commit();
			
			model.getfDivisionJpaService().updateObject(model.getItemHeader());
			
			//UPDATE MANUAL WAJIB HUKUMNYA
			for (FWarehouse fVendor: setFWarehouse) {
				FWarehouse domain = new FWarehouse();
				domain = fVendor;
				domain.setFdivisionBean(model.itemHeader);
				model.getfWarehouseJpaService().updateObject(domain);
...
//		model.getBeanItemContainerBukugiro().addContainerFilter(filter1);
	...
	public DivisionView getView() {
		return view;
	}
	public void setView(DivisionView view) {
		this.view = view;
	}
