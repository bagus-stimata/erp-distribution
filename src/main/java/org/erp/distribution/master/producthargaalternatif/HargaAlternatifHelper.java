...
public class HargaAlternatifHelper {
	
	private HargaAlternatifModel model;
	private HargaAlternatifView view;
	
	public HargaAlternatifHelper(HargaAlternatifModel model, HargaAlternatifView view){
		this.model = model;
		this.view = view;
	}
...
//		model.getBinderHeader().setItemDataSource(model.getItemHeader());
	...
	public List<FtPriceAltd> updateAndCalculateItemDetilFromList(List<FtPriceAltd> list) {
		List<FtPriceAltd> newList = new ArrayList<FtPriceAltd>();
		Iterator<FtPriceAltd> iter = list.iterator();
...
//		}
		...
		Collection itemIds = model.getBeanItemContainerDetil().getItemIds();
		for (Object itemId: itemIds) {
			FtPriceAltd item = new FtPriceAltd();
			item = model.getBeanItemContainerDetil().getItem(itemId).getBean();
...
//		}
	...
	public void postingPerubahanHarga(){
		...
		if (model.getItemHeader().getTrdate().getTime() <= model.getTransaksiHelper().getCurrentTransDate().getTime() ) {
			Iterator<FtPriceAltd> iterFPriced = model.getItemHeader().getFtPricealtdSet().iterator();
			while (iterFPriced.hasNext()) {
				FtPriceAltd ftPriced = new FtPriceAltd();
