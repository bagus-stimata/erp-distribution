...
public class VendorPresenter implements ClickListener, ValueChangeListener, Handler, ItemClickListener{
	private VendorModel model;
	private VendorView view;
	
	public VendorPresenter(VendorModel model, VendorView view){
		this.model = model;
		this.view = view;
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new FVendor();
			//Karena tidak pisa pake null representation
			
			model.newItemHeader.setVcode("New");
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getfVendorJpaService().removeObject(model.getItemHeader());
...
//		}
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getfVendorJpaService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getfVendorJpaService().updateObject(model.getItemHeader());
...
//		Filter filter1 = new SimpleStringFilter("refno", theRefno, true, false);
		Filter filter2 = new Or(new SimpleStringFilter("vcode", theId, true, false));
		Filter filter3 = new Or(new SimpleStringFilter("vname", theDesc, true, false));
//		model.getBeanItemContainerBukugiro().addContainerFilter(filter1);
	...
	public void setView(VendorView view) {
		this.view = view;
	}
