...
public class CustomerPresenter implements ClickListener, ValueChangeListener, Handler, ItemClickListener{
	...
	public void itemClick(ItemClickEvent event) {
		...
		if (entitySelected) {
			...
			model.itemHeader = new FCustomer();
...
//			discardForm(operationStatus);
	...
	public void addItem(){
		try {
			...
			model.newItemHeader = new FCustomer();
			//Karena tidak pisa pake null representation
			
			model.newItemHeader.setCustno("New");
			model.newItemHeader.setCustname("");
			model.newItemHeader.setCreditlimit(0.0);
			model.newItemHeader.setTop(0);
			model.newItemHeader.setOpeninvoice(0);
	        ...
	        ConfirmDialog.Listener konfirmDialogListener = new ConfirmDialog.Listener() {					
				...
				public void onClose(ConfirmDialog dialog) {
                    if (dialog.isConfirmed()) {
                        // Confirmed to continue
                    	try {
                    		model.getfCustomerJpaService().removeObject(model.getItemHeader());
...
//		}
		...
		if (kodeValid && inputValid) {
			try {				
				...
				model.getfCustomerJpaService().createObject(model.getNewItemHeader());
	...
	public int updateAndValidateForm(){
		try{
			...
			model.getfCustomerJpaService().updateObject(model.getItemHeader());
