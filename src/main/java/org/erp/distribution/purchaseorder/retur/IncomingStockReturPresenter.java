...
public class IncomingStockReturPresenter implements ClickListener, ValueChangeListener, Handler, ItemClickListener, BlurListener{
	private IncomingStockReturModel model;
	private IncomingStockReturView view;
	
	IncomingStockReturHelper helper;
	
	public IncomingStockReturPresenter(IncomingStockReturModel model, IncomingStockReturView view){
		this.model = model;
		this.view = view;
		helper = new IncomingStockReturHelper(model, view);
...
//		model.itemDetil.setId(id);
	...
	public void saveAddOrUpdateItemAddFromEditForm(){
		...
		model.getProductAndStockHelper().incomingStockAddRetur(model.getItemHeader().getFwarehouseBean(), listForStockAdd, model.getItemHeader().getPodate());
	...
	public void saveAddOrUpdateItemEditFromEditForm(){
		...
		model.getProductAndStockHelper().incomingStockRemoveRetur(model.getItemHeader().getFwarehouseBean(), listForStockRemove, model.getItemHeader().getPodate());
...
//		model.itemDetil.setId(id);	
                    	...
                    	try {
                    		...
                    		//4. UPDATE STOCK
                        		if (model.getOperationStatus().equals(EnumOperationStatus.EDITING.getStrCode())) {
                        			...
                        			model.getProductAndStockHelper().incomingStockRemoveRetur(model.getItemHeader().getFwarehouseBean(), listForStock, model.getItemHeader().getPodate());
                        		}
	...
	public void saveFormAdding(){
		...
		if (helper.isValidSaveFormAdding()) {
			...
			model.itemHeader.setNopo(model.getTransaksiHelper().getNextFtPurchasehRefnoRetur());
			model.itemHeader.setInvoiceno(model.getTransaksiHelper().getNextFtPurchasehRefnoRetur());
			//3. SAVE ULANG HEADER
			...
			model.getProductAndStockHelper().incomingStockAddRetur(model.getItemHeader().getFwarehouseBean(), listForStock, model.getItemHeader().getPodate());
...
//		}
		...
		parameters.put("paramTipefaktur","R");
	...
	public IncomingStockReturView getView() {
		return view;
	}
	public void setView(IncomingStockReturView view) {
		this.view = view;
	}
