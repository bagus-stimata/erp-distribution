...
public class StockOpnameHelper {
	...
	public StockOpnameHelper(StockOpnameModel model, StockOpnameView view){
		this.model = model;
		this.view = view;
	}
...
//		model.getBinderHeader().setItemDataSource(model.getItemHeader());
	...
	public List<FtOpnamed> updateAndCalculateItemDetilFromList(List<FtOpnamed> list) {
		...
		Iterator<FtOpnamed> iter = list.iterator();
...
//		}
	...
	public void postingParsial(){
		...
		for (Object itemId: itemIds) {
			...
			ftOpnamed = model.getBeanItemContainerDetil().getItem(itemId).getBean();
			//1. AMBIL SALDO AKHIR DAN SESUAIKAN
			Integer qtyTeori = 0;
			...
			fStock.setQtyadjust(qtyPenyesuaian);
			fStock.setSaldoakhir(newQtySaldoStockAkhir);
			
			ftOpnamed.setQtyteori(qtyTeori);
			ftOpnamed.setQtyadjust(qtyPenyesuaian);
