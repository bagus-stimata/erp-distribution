...
public class StockTransferItemPresenter implements ClickListener, BlurListener, ValueChangeListener, Handler, FocusListener{
	private StockTransferItemModel model;
	private StockTransferItemView view;
	private StockTransferItemHelper helper;
	
	public StockTransferItemPresenter(StockTransferItemModel model, StockTransferItemView view){
		this.model = model;
		this.view = view;
		
		helper = new StockTransferItemHelper(model, view);
...
//			helper.validasiHargaBelidanHargaJual();
	...
	public void fixEntityBeforeUpdate(){
		...
		model.ftStocktransferdPK.setId(model.getItemHeader().getRefno());
		model.ftStocktransferdPK.setId(((FProduct) view.getComboProduct().getValue()).getId());
		model.itemDetil.setId(model.ftStocktransferdPK);
