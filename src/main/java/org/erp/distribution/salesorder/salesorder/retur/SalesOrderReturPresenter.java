...
//		model.itemDetil.setId(id);
	...
	public void saveAddOrUpdateItemAddFromEditForm(){
		...
		model.getProductAndStockHelper().soStockAddRetur(model.getItemHeader().getFwarehouseBean(), listForStockAdd, model.getItemHeader().getInvoicedate());
...
//		model.itemDetil.setId(id);	
                    	...
                    	try {
                    		...
                    		//4. UPDATE STOCK
                        		if (model.getOperationStatus().equals(EnumOperationStatus.EDITING.getStrCode())) {
                        			...
                        			model.getProductAndStockHelper().soStockRemoveRetur(model.getItemHeader().getFwarehouseBean(), listForStock, model.getItemHeader().getInvoicedate());
                        		}
	...
	public void saveFormAdding(){
		...
		if (helper.isValidSaveFormAdding()) {
			...
			model.itemHeader.setOrderno(model.getTransaksiHelper().getNextFtSaleshRefnoRetur());
			model.itemHeader.setInvoiceno(model.getTransaksiHelper().getNextFtSaleshInvoicenoRetur());
			//3. SAVE ULANG HEADER
			...
			//REFINE TAMPILAN
			model.getProductAndStockHelper().soStockAddRetur(model.getItemHeader().getFwarehouseBean(), listForStock, model.getItemHeader().getInvoicedate());
	...
	public void cancelFormAdding(){
		...
		model.getFtSaleshJpaService().removeObject(model.itemHeader);
	...
	public void previewInvoice(){
		...
		//SEMENTARA RETUR PAKE STARNDARD REAIL SAJA
//		inputFilePath = "/erp/distribution/reports/salesorder/invoicestd/invoicestd1/invoicestd5.jasper";
...
//		}
		...
		String outputFilePath = "sales_retur";
		...
		String fileName = "salesorderretur_" + outputFilePath + "_" +System.currentTimeMillis()+".pdf";
		StreamResource resource = new StreamResource( source, fileName);
		resource.setMIMEType("application/pdf");
		resource.getStream().setParameter("Content-Disposition","attachment; filename="+fileName);		
		
		view.getUI().getPage().open(resource, "_new_print_retur_" + outputFilePath, false);
