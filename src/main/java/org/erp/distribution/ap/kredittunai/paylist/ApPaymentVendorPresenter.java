...
public class ApPaymentVendorPresenter implements ClickListener, ValueChangeListener, Handler {
	/**
	 * 
	 */
	...
	private ApPaymentVendorView view;
	
	public ApPaymentVendorPresenter(ApPaymentVendorModel model, ApPaymentVendorView view){
		this.model = model;
		this.view = view;
...
//		view.getPanelForm().addActionHandler(this);
			...
			public void buttonClick(ClickEvent event) {
				...
				if (view.getFormModel().isAllowCloseWindow()==true){
					...
					if(view.getFormModel().getFormOperationStatus().equals(EnumOperationStatus.ADDING.getStrCode())){
						model.getBeanItemContainerHeader().addBean(view.getFormModel().getApPaymentHeader());
					}
	...
	ItemClickListener listenerItemClickHeader =  new ItemClickListener() {
		...
		public void itemClick(ItemClickEvent event) {
			...
			model.itemHeader = new FtAppaymenth();
...
//			String div = model.getItemInvoice().getId().getDivision();
			...
			model.beanItemContainerDetail = new BeanItemContainer<FtAppaymentd>(FtAppaymentd.class);
...
//			listDetail = model.getArPaymentDetailService().findAllByRefnoAndInvAndDiv(refno, inv, null);
			listDetail = model.getApPaymentDetailService().findAllByRefnoAndInvAndDiv(refnopayment, refnosales, null);
			...
			for (FtAppaymentd item: listDetail){
				double subtotal = item.getCashamountpay() + item.getGiroamountpay() +
						item.getMrvamountpay() + item.getTransferamountpay() + item.getDcvamountpay();
	...
	private ValueChangeListener listenerTableDetail = new ValueChangeListener() {		
		...
		public void valueChange(ValueChangeEvent event) {
			...
			model.itemDetail = new FtAppaymentd();
			try{
...
//				model.getItemHeader().setId(id);
				...
				model.beanItemHeader = new BeanItem<FtAppaymenth>(model.getItemHeader());
				
				model.itemDetail = new FtAppaymentd();
				FtAppaymentdPK itemDetailPK = new FtAppaymentdPK();
				itemDetailPK.setRefnopayment((long)0);
				itemDetailPK.setRefnopurchase(model.getItemInvoice().getRefno());
...
//				model.getItemDetail().setDivisionBean(model.getItemInvoice().getDivisionBean());
	...
	public void deleteItem(final FtAppaymentd appaymentdetail) {
		if (appaymentdetail != null){			
                    ...
                    if (dialog.isConfirmed()) {
                    	...
                    	try {
                    		...
                    		double amountThisDetail = model.getItemDetail().getCashamountpay() + 
                    				...
                    				model.getItemDetail().getMrvamountpay() + 
                    				model.getItemDetail().getTransferamountpay() + 
                    				model.getItemDetail().getDcvamountpay() + 
